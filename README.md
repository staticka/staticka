...
``` bash
$ composer require staticka/staticka illuminate/view
```

**NOTE**: [Laravel Blade](https://laravel.com/docs/5.5/blade) is only available in PHP v5.4.0 and later. For PHP v5.3.0, you can use [Twig](https://twig.symfony.com/) as a replacement (`twig/twig`) or the `Renderer` instance (the default one) from Slytherin.
...
The contents of the `routes.php` file must return an array of `Staticka\Route` instances:
...
array_push($routes, new Staticka\Route('/', 'hello-world'));
array_push($routes, new Staticka\Route('/hello-world', 'hello-world'));
...
return array(
     ...
     */
    'integrations' => array(
        'Staticka\Content\MarkdownIntegration',
        'Staticka\Helper\HelperIntegration',
        'Staticka\Renderer\RendererIntegration',
    ),
);
```
...
If you have a filter or a helper that contains dependencies or needs to be configured first, it is better to add them into an integration in order for Staticka to use it properly. The example below contains the integration file of the `Staticka\Helper\UrlHelper`:
